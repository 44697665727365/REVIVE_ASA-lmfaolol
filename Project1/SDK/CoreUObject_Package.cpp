...
namespace CG
{
	 ...
	 * 		FVector&                                           v
	 */
	double FVector::DistanceMeter(const FVector& v) const
	{
	...
	FLinearColor::FLinearColor(float r, float g, float b)
	{
		R = r;
		G = g;
		B = b;
	...
	FLinearColor::FLinearColor(int r, int g, int b, int a)
	{
		R = float(r);
		G = float(g);
		B = float(b);
	...
	FLinearColor::FLinearColor(int r, int g, int b)
	{
		R = r / 255.0f;
		G = g / 255.0f;
		B = b / 255.0f;
	...
	FLinearColor::FLinearColor(int r, int g, int b, float a)
	{
		R = r / 255.0f;
		G = g / 255.0f;
		B = b / 255.0f;
	...
	double FLinearColor::RGBToLinear(uint8_t color)
	{
	...
	FLinearColor FLinearColor::FromRGB(int R, int G, int B)
	{
		return FLinearColor{ (float)RGBToLinear(R), (float)RGBToLinear(G), (float)RGBToLinear(B), 1.0f };
	}

	FLinearColor FLinearColor::FromARGB(int A, int R, int G, int B)
	{
		return FLinearColor{ (float)RGBToLinear(R), (float)RGBToLinear(G), (float)RGBToLinear(B), (float)(A / 255.0f) };
	}
	FLinearColor FLinearColor::LerpUsingHSV(FLinearColor A, FLinearColor B, float alpha)
	{
		FLinearColor ret;
		ret.R = A.R + alpha * (B.R - A.R);
		ret.G = A.G + alpha * (B.G - A.G);
		ret.B = A.B + alpha * (B.B - A.B);
		ret.A = A.A + alpha * (B.A - A.A);
		return ret;
	}
	/**
	 * Function:
	...
	{
		return TypeFlag != EClassCastFlags::CASTCLASS_None ? Class->CastFlags & TypeFlag : true;
	}
	bool FInternalUObjectBaseUtilityIsValidFlagsChecker::CheckObjectValidBasedOnItsFlags(const UObject* Test)
	{
		//return !((Test->Flags & RF_InternalPendingKill | RF_InternalGarbage) != 0);
		return !Test->HasAnyFlags(EObjectFlags(RF_InternalPendingKill | RF_InternalGarbage));
	}
	/**
	 * Function:
	 * 		RVA    -> 0x00000000
	...
	{
		return GetVFunction<UObject*(*)(UClass*)>(this, 0x0)(this);
	}
