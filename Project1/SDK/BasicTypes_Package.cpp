...
namespace CG
{
	...
	//bool InitSdk()
	//{
	//	return InitSdk(L"ArkAscended.exe", 0x96DA690, 0x99AB480, 0x9BED7C0);
	//}
	...
	{
		UObject* ItemPtr = GetObjectPtr((uint32_t)index);
	...
	{
		FUObjectItem* ItemPtr = (FUObjectItem*)GetObjectPtr((uint32_t)index);
		if (!ItemPtr) return nullptr;
		return ItemPtr;
	}
	...
	//	if (!AppendString)
	//		AppendString = reinterpret_cast<void(*)(const FName*, FString&)>(uintptr_t(this->AppendStringOff));
