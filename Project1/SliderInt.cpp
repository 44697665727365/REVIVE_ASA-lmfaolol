...
void SliderInt::HandleSlider()
{
	...
	if (SliderBase::DraggingSlider == this)
	{
		...
		*this->Value = (int)((float)(cursorPos.X - sliderBeginPos.X) / oneP) + this->minValue;

		*this->Value = std::clamp(*this->Value, this->minValue, this->maxValue);
	}
}

FVector2D SliderInt::GetSliderBeginPos()
{
